...
describe("Given I am a user connected as Admin", () => {
  describe("When I navigate to Dashboard", () => {
    test("fetches bills from mock API GET", async () => {
      // simulation de la connexion d'un admin
      localStorage.setItem(
        "user",
        JSON.stringify({ type: "Admin", email: "a@a" })
      );
      // on crée un div pour que le dashboard s'affiche
      const root = document.createElement("div");
      root.setAttribute("id", "root");
      document.body.append(root);
      // on appelle le router pour afficher le dashboard
      router();
      // on attend que le dashboard s'affiche
      window.onNavigate(ROUTES_PATH.Dashboard);
      // on attend que le tableau de factures s'affiche
      await waitFor(() => screen.getByText("Validations"));
      // on vérifie que les données de l'api sont bien affichées
